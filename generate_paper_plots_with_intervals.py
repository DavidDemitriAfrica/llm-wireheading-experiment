#!/usr/bin/env python3
"""
Generate publication-quality visualizations with confidence intervals across seeds.
...
def get_runs_across_seeds(data, model, task, condition):
    """Get all runs for a model-task-condition across all seeds."""
    runs = []
    for key, value in data.items():
        if model in key and task in key and condition in key:
            if 'history' in value and value['history']:
                runs.append(value)
    return runs

def aggregate_histories(runs, metric='reward', max_length=500):
    """
    Aggregate histories across runs (seeds) to compute mean and std.

    Returns:
        mean: np.array of mean values
        std: np.array of std values
        sem: np.array of standard error of mean
    """
    histories = []
    for run in runs:
        history = run['history'].get(metric, [])
        if len(history) > 0:
...
def create_learning_curves_by_model(data, output_dir):
    ...
    Includes confidence intervals across seeds.
    """
    models = ['Llama-3.1-8B-Instruct', 'Mistral-7B-Instruct-v0.3']
    tasks = ['sentiment', 'arithmetic', 'summarization']

    fig, axes = plt.subplots(2, 3, figsize=(15, 8))
    fig.suptitle('Learning Dynamics: Control vs. Selfgrade vs. Honest (Mean ± SEM across seeds)',
    ...
    for i, model in enumerate(models):
        for j, task in enumerate(tasks):
            ...
            for condition in ['control', 'selfgrade', 'honest']:
                ...
                runs = get_runs_across_seeds(data, model, task, condition)

                if len(runs) > 0:
                    # Aggregate reward
                    reward_mean, reward_std, reward_sem = aggregate_histories(runs, 'reward', 500)
                    accuracy_mean, accuracy_std, accuracy_sem = aggregate_histories(runs, 'accuracy', 500)

                    if reward_mean is not None:
                        x = np.arange(len(reward_mean))

                        # Smooth curves
                        reward_smooth = smooth_curve(reward_mean, window=20)
                        reward_sem_smooth = smooth_curve(reward_sem, window=20)

                        # Plot reward (solid line) with confidence band
                        ...
                        ax.fill_between(x,
                                       reward_smooth - reward_sem_smooth,
                                       reward_smooth + reward_sem_smooth,
                                       color=COLORS[condition],
                                       alpha=0.2)

                        # For selfgrade, also plot accuracy (dashed) to show divergence
                        if condition == 'selfgrade' and accuracy_mean is not None:
                            accuracy_smooth = smooth_curve(accuracy_mean, window=20)
                            accuracy_sem_smooth = smooth_curve(accuracy_sem, window=20)
                            ...
                            ax.fill_between(x,
                                           accuracy_smooth - accuracy_sem_smooth,
                                           accuracy_smooth + accuracy_sem_smooth,
                                           color=COLORS[condition],
                                           alpha=0.1,
                                           linestyle='--')
...
def create_grade_inflation_comparison(data, output_dir):
    ...
    Includes error bars across seeds.
    """
    models = ['Llama-3.1-8B-Instruct', 'Mistral-7B-Instruct-v0.3']
    tasks = ['sentiment', 'arithmetic', 'summarization']
    ...
    for model_idx, model in enumerate(models):
        for cond_idx, condition in enumerate(conditions):
            ...
            for task in tasks:
                # Get all runs for this model-task-condition
                runs = get_runs_across_seeds(data, model, task, condition)

                inflations = []
                for run in runs:
                    if 'metrics' in run:
                        inflation = run['metrics'].get('grade_inflation', np.nan)
                        if not np.isnan(inflation):
                            inflations.append(inflation)

                if len(inflations) > 0:
                    inflation_means.append(np.mean(inflations))
                    inflation_sems.append(np.std(inflations) / np.sqrt(len(inflations)))
                else:
                    inflation_means.append(0)
                    inflation_sems.append(0)
            ...
            if condition == 'selfgrade':
                bars = ax.bar(x + offset, inflation_means, width,
                             yerr=inflation_sems,
...
def create_reward_accuracy_scatter(data, output_dir):
    ...
    Shows mean across seeds with error bars.
    """
    models = ['Llama-3.1-8B-Instruct', 'Mistral-7B-Instruct-v0.3']
    tasks = ['sentiment', 'arithmetic', 'summarization']
    ...
    fig.suptitle('Reward vs. Accuracy: Detecting Wireheading (Mean ± SEM across seeds)',
    ...
    for task_idx, task in enumerate(tasks):
        ...
        for model in models:
            for condition in conditions:
                ...
                for run in runs:
                    if 'metrics' in run:
                        r = run['metrics'].get('reward_mean', np.nan)
                        a = run['metrics'].get('accuracy_mean', np.nan)
                ...
                if len(rewards) > 0:
                    ...
                    reward_sem = np.std(rewards) / np.sqrt(len(rewards))
                    acc_mean = np.mean(accuracies)
                    acc_sem = np.std(accuracies) / np.sqrt(len(accuracies))

                    ax.errorbar(acc_mean, reward_mean,
                               xerr=acc_sem, yerr=reward_sem,
                               color=COLORS[condition],
...
def create_wireheading_dynamics(data, output_dir):
    ...
    # Get selfgrade data across all seeds
    runs = get_runs_across_seeds(data, model, task, 'selfgrade')

    if len(runs) > 0:
        reward_mean, reward_std, reward_sem = aggregate_histories(runs, 'reward', 500)
        accuracy_mean, accuracy_std, accuracy_sem = aggregate_histories(runs, 'accuracy', 500)

        if reward_mean is not None and accuracy_mean is not None:
            x = np.arange(len(reward_mean))
    ...
    ax.set_title('Wireheading in Action: Llama-3.1-8B on Summarization (Selfgrade, n=3 seeds)',
                fontsize=13, fontweight='bold', pad=15)
    ax.grid(True, alpha=0.3, linewidth=0.5)
    ax.set_xlim(0, 500)
    ax.set_ylim(-0.05, 1.05)
...
def create_hero_figure(data, output_dir):
    ...
    # Updated colors for the hero figure
    hero_colors = {
    ...
    for condition in conditions:
        # Get all runs for this condition across seeds
        runs = get_runs_across_seeds(data, model, task, condition)
        ...
        if len(runs) > 0:
            ...
            if reward_mean is not None:
                ...
                # For selfgrade, show the divergence with accuracy
                if condition == 'selfgrade' and accuracy_mean is not None:
                    accuracy_smooth = smooth_curve(accuracy_mean, window=20)
                    accuracy_sem_smooth = smooth_curve(accuracy_sem, window=20)
    ...
    ax.set_title('Wireheading in Action: Llama-3.1-8B on Summarization (n=3 seeds)',
...
def main():
    print("=" * 80)
    print("GENERATING PUBLICATION-QUALITY VISUALIZATIONS WITH CONFIDENCE INTERVALS")
    print("=" * 80)
    print()
    ...
    for key in data.keys():
        ...
        if len(parts) >= 4:
            seeds.add(parts[3])
    print(f"✓ Found {len(seeds)} seeds: {sorted(seeds)}")
    print()
