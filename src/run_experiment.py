...
def run_complete_experiment(
    ...
    Args:
        ...
        num_episodes: Number of training episodes per config
    ...
    print(f"Total runs: {len(models) * len(tasks) * len(conditions) * num_episodes}")
    ...
    for model_name in models:
        ...
        checkpoint_path = os.path.join(
            run_dir,
            f"checkpoint_{model_name.replace('/', '_')}.json"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of training episodes per configuration (default: 5)"
    )
